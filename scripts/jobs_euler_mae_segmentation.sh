...
for MASK in "${MASKs[@]}"
do
    RUN_TAG="${DATASET}_${STRATEGY}_pixel_${MASK}"
    NAME="/cluster/home/callen/projects/mae/output_log/$RUN_TAG"
    JOB="python main.py user=callen_euler experiment=$DATASET masking=$STRATEGY masking.pixel_ratio=$MASK run_tag=$RUN_TAG"
    echo $JOB
