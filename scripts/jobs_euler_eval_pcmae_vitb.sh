...
DATASETs=(pcmae_blood_pc pcmae_cifar10_pc pcmae_derma_pc pcmae_tiny_pc pcmae_path_pc)
MODEL="vit-b"
...
# NAME="/cluster/home/abizeul/mae/output_log/$RUN_TAG"
# JOB="python main.py user=abizeul_euler experiment=$DATASET masking.pc_ratio=$MASK trainer=eval_knn checkpoint=pretrained model=$MODEL checkpoint.epoch=$EPOCH run_tag=$RUN_TAG"
# sbatch -o "$NAME" -n 1 --cpus-per-task "$NUM_WORKERS" --mem-per-cpu="$MEM_PER_CPU" --time="$TIME" -p gpu --gpus=1 --gres=gpumem:"$MEM_PER_GPU" --wrap="nvidia-smi;$JOB"
done; done; done;
