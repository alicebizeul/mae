...
from mae.utils_old import save_reconstructed_images, save_attention_maps, get_eigenvalues, LinearWarmupScheduler, PairedImageDataset
import random
...
def parse_args():
    ...
    parser.add_argument('--eval_epochs', type=int, default=100, help='Number of epochs to train the model.')
    parser.add_argument('--eval_freq', type=int, default=100, help='Number of epochs to train the model.')
    ...
    # adjusting the dataset dependant variables: channels, resolution
...
def main():
    ...
    if args.facebook:
        ...
        [
            ...
            transforms.RandomResizedCrop(64, scale=(0.2, 1.0), interpolation=InterpolationMode.BICUBIC),
    ...
    if args.arch == "vit_t":
        ...
        config_eval = ViTMAEConfig(hidden_size=192,num_attention_head=3,intermediate_size=768,image_size=64,patch_size=args.patch_size,mask_ratio=0.0,norm_pix_loss=True,attn_implementation="eager")
        ...
        config_eval = ViTMAEConfig(hidden_size=384,num_attention_head=6,intermediate_size=1536,image_size=64,patch_size=args.patch_size,mask_ratio=0.0,norm_pix_loss=True,attn_implementation="eager")
        ...
        config_eval = ViTMAEConfig(hidden_size=768,num_attention_head=12,intermediate_size=1536,image_size=64,patch_size=args.patch_size,mask_ratio=0.0,norm_pix_loss=True,attn_implementation="eager")
    else: 
    ...
    for epoch in range(args.epochs):
        if not args.eval:
            ...
            for batch_index, (batch1, batch2) in enumerate(trainloader):
                ...
                loss = model.forward_loss(batch2,outputs.logits,outputs.mask)
        ...
        if epoch%args.eval_freq==0 or (epoch+1)==args.epochs:
            ...
            if args.facebook:
                ...
                model_eval = ViTMAEModel(config_eval)
