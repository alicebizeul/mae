...
import torch
from transformers import ViTMAEConfig, ViTMAEPreTrainedModel
...
class ViTMAEEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.position_embeddings = nn.Parameter(
            torch.zeros(1, self.image_size*self.image_size*self.num_channels + 1, config.hidden_size), requires_grad=False
        )
    ...
    def initialize_weights(self):
        ...
        pos_embed = get_2d_sincos_pos_embed(
            self.position_embeddings.shape[-1], int(self.image_size*self.image_size*self.num_channels), add_cls_token=True
        )
...
class ViTMAEPatchEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        num_channels, self.hidden_size = config.num_channels, config.hidden_size
        image_size = image_size if isinstance(image_size, collections.abc.Iterable) else (image_size, image_size)
        patch_size = patch_size if isinstance(patch_size, collections.abc.Iterable) else (patch_size, patch_size)
        num_patches = len(patch_size)
        ...
        self.projection = dict([(p,nn.Conv1d(1, self.hidden_size, kernel_size=p, stride=p)) for p in patch_size])
    ...
    def forward(self, pixel_values, interpolate_pos_encoding: bool = False):
        ...
        #     )
        self.num_patches = len(pixel_values)
        x = [self.projection[p.shape[2]](p).transpose(1, 2) for p in pixel_values]
...
class ViTMAEDecoder(nn.Module):
    def __init__(self, config, num_patches):
        ...
        self.decoder_pred = dict([(p,nn.Linear(
            config.decoder_hidden_size, p, bias=True
        )) for p in config.patch_size])  # encoder to decoder
        self.gradient_checkpointing = False
        self.config = config
    ...
    def initialize_weights(self, num_patches):
        ...
        decoder_pos_embed = get_2d_sincos_pos_embed(
            self.decoder_pos_embed.shape[-1], int(num_patches), add_cls_token=True
        )
