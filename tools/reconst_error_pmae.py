...
# model 
vit_config  = transformers.ViTMAEConfig(hidden_size=192,num_attention_head=3,intermediate_size=768,norm_pix_loss=False,attn_implementation="eager",mask_ratio=0.0,patch_size=4,image_size=32)
...
checkpoint = "/cluster/scratch/abizeul/mae_alice/logs/abizeul/pcmae_cifar10_pc_pc_pc_0.1_lossA/2025-01-28_10-24-53/checkpoints/epoch=999-train_loss=0.00.ckpt"
...
masking_fn_=torch.tensor(np.load("/cluster/scratch/abizeul/cifar-10-batches-py/pc_matrix.npy")).T
eigenvalues=torch.tensor(np.load("/cluster/scratch/abizeul/cifar-10-batches-py/eigenvalues_ratio.npy"))
# # 
...
for i, (batch,_) in enumerate(datamodule.val_dataloader()):
    ...
    threshold = find_threshold(eigenvalues[index],0.1)
    pc_mask = torch.tensor(index[:threshold])
    indexes = torch.arange(masking_fn_.shape[1])
    pc_mask_input = indexes[~torch.isin(indexes,pc_mask[pc_mask!=-1])]

    target  = (batch.reshape([batch.shape[0],-1]) @ masking_fn_[:,pc_mask]@ masking_fn_[:,pc_mask].T).reshape([64,3,32,32])
    img     = (batch.reshape([batch.shape[0],-1]) @ masking_fn_[:,pc_mask_input] @ masking_fn_[:,pc_mask_input].T).reshape([64,3,32,32])

    outputs, cls = model_train.model(img,return_rep=False)
    reconstruction = model_train.model.unpatchify(outputs.logits)
    reconstruction = (reconstruction.reshape([img.shape[0],-1]) @ masking_fn_[:,pc_mask]@ masking_fn_[:,pc_mask].T).reshape([64,3,32,32])
    ...
    loss+=torch.nn.MSELoss()(target,reconstruction)
    ...
    if i ==0:
        ...
        for j in range(num_images):
            ...
            # Reconstruction
            recon_img = tensor_to_image(reconstructions[j])
            ...
            recon_img = tensor_to_image(target[j])
            axes[3,j].imshow(recon_img)
            if j==0:axes[3,j].set_ylabel("Target")
            axes[3,j].axis("off")
            ...
            masked_img = tensor_to_image(img[j])
            axes[1,j].imshow(masked_img)
            if j==0:axes[1,j].set_ylabel("Model Input")
            axes[1,j].axis("off")
