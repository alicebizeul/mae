...
def plot_data_plotly(epochs, pmae, mae, title, position):
    ...
    fig.update_layout(
        ...
        xaxis_title="Epochs",
        yaxis_title="Lin. Acc. (%)",
        font=dict(size=12),  # Font size for all text
        ...
        xaxis=dict(showgrid=True, tickfont=dict(size=14), gridcolor='gray'),  # X-axis ticks and grid
        yaxis=dict(showgrid=True, tickfont=dict(size=14), gridcolor='gray')   # Y-axis ticks and grid
    )
    
    fig.write_image(f"./{title}.png", scale=2, width=300, height=300)
    fig.write_image(f"./{title}.pdf", scale=2, width=300, height=300)
    # fig.show()
